...
function normalizeSchoolOrigin(url) {
  try {
...
function studentSessionKey(session) {
...
function mergeStudentSessionGroup(sessions) {
...
function collapseSessionsByStudent(sessions, activeSessionId) {
  const list = Array.isArray(sessions) ? sessions.filter(Boolean) : []
...
function applyCredentialToSession(session, credential) {
...
module.exports = {
  normalizeStudentNo,
  normalizeSchoolOrigin,
  studentSessionKey,
  mergeStudentSessionGroup,
